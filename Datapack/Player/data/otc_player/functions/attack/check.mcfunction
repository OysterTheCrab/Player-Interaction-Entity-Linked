...
execute if score $differs otcInteract matches 0 at @s as @a[tag=otcInteract,limit=1,predicate=otc_player:match_id] run function otc_player:attack/reward

# Runs the error function if the UUID matches and they are not linked
execute if score $differs otcInteract matches 0 at @s as @a[tag=otcInteract,limit=1,predicate=!otc_player:match_id] run function otc_player:attack/error
